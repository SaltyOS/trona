// Memory manager server (mmsrv) IPC protocol labels (0x80-0xA3 range, gap at 0x91).
...
pub const MM_SHM_DESTROY: u64 = 0x8E;
pub const MM_SHM_RESIZE: u64 = 0x8F;
...
// 0x98, 0x99 removed: mmsrv no longer owns an untyped pool, so the
// procmgr→mmsrv push-provision path (MM_PROVISION_UNTYPED) and the
// capacity query (MM_QUERY_CAPACITY) are gone. Numbers reserved.
...
pub const MM_PREFAULT_RANGE: u64 = 0xA4;
pub const MM_MPROTECT_TARGET: u64 = 0xA5;
pub const MM_ALLOC_TYPED_COPY_FROM_CLIENT_REGION: u64 = 0xA6;
